...
joblib.dump(clf, "../trained/MNB_sentiment_trained.joblib")
...
write_to_performance_file(PERFORMANCE_FILE_PATH, MNB, CLASSIFIER_SENTIMENT, confusion_matrix, cl_report)

# For Emotion
X_train, X_test, y_train, y_test = train_test_split(df_counts, targets_emotion, test_size=0.2, random_state=0)
clf = MultinomialNB()
clf.fit(X_train, y_train)
joblib.dump(clf, "../trained/MNB_emotion_trained.joblib")
...
write_to_performance_file(PERFORMANCE_FILE_PATH, MNB, CLASSIFIER_EMOTION, confusion_matrix, cl_report)
